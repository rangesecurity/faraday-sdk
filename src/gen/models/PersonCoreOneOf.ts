/* tslint:disable */
/* eslint-disable */
/**
 * Faraday API
 *  The **Faraday API** provides secure, compliant infrastructure for stablecoin transactions, identity verification, and transaction monitoring across multiple blockchains.  ### Key Features - **Swap Routing** — Discover optimal swap paths for stablecoin transfers, including cross-chain routes. - **IVMS101 Person Records** — Create, query, and manage structured person data for FATF Travel Rule compliance. - **Transaction Submission and Monitoring** — Submit and track stablecoin transactions. - **Reporting** — View historical transaction reports including associated IVMS101 records.  ### Authentication Most requests require an API key (Bearer token).  Generate and manage API keys by signing in to: - Production: https://app.range.org/ - Staging: https://stage.range.org/  API Keys can be found under the APIs section. A free monthly credit allowance is available, with more credits available to paid plans.  Include this header with every authenticated Faraday request:  ``` Authorization: Bearer <your_token> ```  ### Base URLs - Production: `https://api.faraday.range.org` - Staging: `https://faraday.api.rase.live` - Local Development: `http://0.0.0.0:3000`  ### Example Request ```bash curl -X GET \'https://api.faraday.range.org/v1/persons\'   --header \'Authorization: Bearer <your_token>\' ```         
 *
 * The version of the OpenAPI document: 1.0.0
 * Contact: faraday@range.org
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import { mapValues } from '../runtime';
import type { PersonCoreOneOfPerson } from './PersonCoreOneOfPerson';
import {
    PersonCoreOneOfPersonFromJSON,
    PersonCoreOneOfPersonFromJSONTyped,
    PersonCoreOneOfPersonToJSON,
    PersonCoreOneOfPersonToJSONTyped,
} from './PersonCoreOneOfPerson';

/**
 * 
 * @export
 * @interface PersonCoreOneOf
 */
export interface PersonCoreOneOf {
    /**
     * 
     * @type {string}
     * @memberof PersonCoreOneOf
     */
    kind: PersonCoreOneOfKindEnum;
    /**
     * 
     * @type {PersonCoreOneOfPerson}
     * @memberof PersonCoreOneOf
     */
    person: PersonCoreOneOfPerson;
}


/**
 * @export
 */
export const PersonCoreOneOfKindEnum = {
    natural: 'natural'
} as const;
export type PersonCoreOneOfKindEnum = typeof PersonCoreOneOfKindEnum[keyof typeof PersonCoreOneOfKindEnum];


/**
 * Check if a given object implements the PersonCoreOneOf interface.
 */
export function instanceOfPersonCoreOneOf(value: object): value is PersonCoreOneOf {
    if (!('kind' in value) || value['kind'] === undefined) return false;
    if (!('person' in value) || value['person'] === undefined) return false;
    return true;
}

export function PersonCoreOneOfFromJSON(json: any): PersonCoreOneOf {
    return PersonCoreOneOfFromJSONTyped(json, false);
}

export function PersonCoreOneOfFromJSONTyped(json: any, ignoreDiscriminator: boolean): PersonCoreOneOf {
    if (json == null) {
        return json;
    }
    return {
        
        'kind': json['kind'],
        'person': PersonCoreOneOfPersonFromJSON(json['person']),
    };
}

export function PersonCoreOneOfToJSON(json: any): PersonCoreOneOf {
    return PersonCoreOneOfToJSONTyped(json, false);
}

export function PersonCoreOneOfToJSONTyped(value?: PersonCoreOneOf | null, ignoreDiscriminator: boolean = false): any {
    if (value == null) {
        return value;
    }

    return {
        
        'kind': value['kind'],
        'person': PersonCoreOneOfPersonToJSON(value['person']),
    };
}

